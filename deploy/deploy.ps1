...
write_files:
  ...
  - path: /tmp/photo_ai.env
    ...
    content: |
      ...
      OLLAMA_BASE_URL=$(if ($OLLAMA_BASE_URL) { $OLLAMA_BASE_URL } else { 'http://172.24.24.91:11434' })
      OLLAMA_EMBED_MODEL=$(if ($OLLAMA_EMBED_MODEL) { $OLLAMA_EMBED_MODEL } else { 'bge-m3' })
      SEARCH_SIMILARITY_FLOOR=$(if ($SEARCH_SIMILARITY_FLOOR) { $SEARCH_SIMILARITY_FLOOR } else { '0.40' })
      SEARCH_RELATIVE_CUTOFF=$(if ($SEARCH_RELATIVE_CUTOFF) { $SEARCH_RELATIVE_CUTOFF } else { '0.90' })
